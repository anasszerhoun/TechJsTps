...
async function updateBookPages(id, pagesRead) {
    try {
...
form.onsubmit = async (e) => {
    ...
    const statusString = formData.get("status");
    const formatString = formData.get("format");
    const finishedCheckbox = formData.get("finished");
    const pagesReadValue = formData.get("pagesRead");
    const bookData = {
    ...
    const book = new Book(bookData.title, bookData.author, bookData.pages, BookStatus[statusString], bookData.price, bookData.pagesRead, BookFormat[formatString], bookData.suggestedBy, bookData.finished);
...
async function displayBooks() {
    try {
      ...
      `;
        ...
        booksListContainer.innerHTML = '';
        books.forEach((book) => {
            const bookInstance = new Book(book.title, book.author, book.pages, book.status, book.price, book.pagesRead, book.format, book.suggestedBy, book.finished, book._id);
      ...
      `;
            ...
            const updatePagesBtn = bookCard.querySelector('.update-pages-btn');
            updatePagesBtn.addEventListener('click', async () => {
                ...
                const newPagesRead = prompt(`Modifier le nombre de pages lues (max: ${totalPages}):`, currentPages.toString());
